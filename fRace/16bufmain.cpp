...
//sektor ref[n] do b[m]
void LoadSector(MAP *map,int n,int m,int x,int y)
{
 int i,j;
 unsigned char *wsk;
 int d=map->s.d;
 if ((x<0)||(y<0)||(x>map->w/d)||(y>map->h/d))
 {
  unsigned char v,t;
  v=ESand;
  i=d;
 ...
 while (j--)
 {
  fseek(f,(y0*map->w+x0)/2,SEEK_SET);
  i=d/2;
  while (i--)
  {
   unsigned char v;
   fread(&v,1,1,f);
   *(wsk++)=(v&0xf)|((rand()%MAXBMPS)<<4);
   *(wsk++)=(v>>4)|((rand()%MAXBMPS)<<4);
  }
  y0++;
 }
 fclose(f);
...
void LoadMap(char *mapname)
{
 ...
 fread(&w,4,1,f);
 fread(&h,4,1,f);
 carposx=(cpx-w/2)*CELLINPIX/METERINPIX;
 carposy=(-cpy+h/2)*CELLINPIX/METERINPIX;
 carangle=ca;
 map.w=w;
 map.h=h;
 if (map.b==0)
 map.b=(unsigned char *)malloc(w*h);
 else
 map.b=(unsigned char *)realloc(map.b,w*h);
 for (i=0; i<w*h/2; i++)
 {
  unsigned char v;
  fread(&v,1,1,f);
