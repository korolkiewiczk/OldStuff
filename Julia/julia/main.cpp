...
void julia(float x_re,float x_im,float c_re,float c_im,float d_re,float d_im,void *buf,int h)
{
 int x,y,c;
 float x_re2;
...
void juliaint(float fx_re,float fx_im,float fc_re,float fc_im,float fd_re,float fd_im,void *buf,int h)
{
 #define SHVAL 13
...
DWORD WINAPI _juliathreaded(LPVOID i)
{
 ...
 d_im=tfd_im;
 if (method==0)
 julia(x_re,x_im,c_re,c_im,d_re,d_im,bufr[(int)i],h0);
 else
 if (method==1)
 juliaint(x_re,x_im,c_re,c_im,d_re,d_im,bufr[(int)i],h0);
 ExitThread(0);
 return 0;
}
...
void juliathreaded(float fx_re,float fx_im,float fc_re,float fc_im,float fd_re,float fd_im)
{
 ...
 tfd_re=fd_re;
 tfd_im=fd_im;
 h0=SCRHEIGHT/numofthreads;
 for (lpthread=0; lpthread<numofthreads; lpthread++)
 {
  tfx_im[lpthread]=fx_im;
  bufr[lpthread]=(void*)((int)bf0+4*(SCRWIDTH*(h0*lpthread)));
  h[lpthread]=CreateThread(0,0,_juliathreaded,(LPDWORD)lpthread,0,0);
  fx_im+=((float)h0*fd_im);
 }
 WaitForMultipleObjects(numofthreads,(HANDLE*)&h,TRUE,INFINITE);
...
int stop=0;
#define METHODS 2
char *methodnames[METHODS]={"Dokladna(FPU)","Szybka(CPU-INT32)"};
int vcol=0;
...
int main()
{
 ...
 while (1)
 {
  ...
  (float)(MAXXRE-MINXRE)/(float)SCRWIDTH,(float)(MAXXIM-MINXIM)/(float)SCRHEIGHT,buf,SCRHEIGHT);
  else
  if (method==1)
